...
class TouchImageView @JvmOverloads constructor(
    ...
    private var delayedZoomVariables: ZoomVariables? =
        null
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        viewWidth = totalViewWidth - paddingLeft - paddingRight
        viewHeight = totalViewHeight - paddingTop - paddingBottom
...
//        	if (prevMatchViewWidth == 0 || prevMatchViewHeight == 0) {
    ...
    override fun canScrollVertically(direction: Int): Boolean {
        ...
        if (imageHeight < viewHeight) {
            ...
            return false
        } else if (Math.abs(y) + viewHeight + 1 >= imageHeight && direction > 0) {
            return false
        }
        return true
    }
    ...
    private inner class GestureListener : SimpleOnGestureListener() {
        ...
        override fun onDoubleTap(e: MotionEvent): Boolean {
            ...
            if (state == State.NONE) {
                val targetZoom =
                    if (currentZoom == minScale) maxScale else minScale
                val doubleTap =
                    DoubleTapZoom(
                        targetZoom,
                        e.x,
